...
matrix matrix::Inv()
{
	int n = rows;
    int lda = ((n+15)&~15|16);
    double * AI = (double *)malloc(sizeof(double)*(n*lda*2));
    memset(AI,0,sizeof(double)*n*lda*2);
    for (int i = 0; i < n; i++) {
        memcpy(&AI[lda*i*2], (double *)&p[n*i], sizeof(double)*n);
        AI[lda*i*2+n+i] = 1;
    }

    double * AI_d;
    cudaMalloc((void **) &AI_d, sizeof(double)*n*lda*2);
    cudaMemcpy(AI_d, AI, sizeof(double)*n*lda*2, cudaMemcpyHostToDevice);
    invertge(AI_d, lda, n);
    cudaMemcpy(AI, AI_d, sizeof(double)*n*lda*2, cudaMemcpyDeviceToHost);
    cudaFree(AI_d);
   
    for (int i = 0; i < n; i++) {
    memcpy((double *)&p[n*i], &AI[lda*i*2+n], sizeof(double)*n);
    }
    free(AI);
