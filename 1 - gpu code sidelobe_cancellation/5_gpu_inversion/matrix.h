...
public:
	...
	bool matrixSpaceHasBeenAllocated;
	  ...
	  matrix(const int row_count, const int column_count, double *vector);
	...
	//static matrix operator_mult (const matrix* a, const matrix* b);
	friend matrix operator* (const matrix& a, const matrix& b);
  ...
  int GetCols() const;
	...
	double* GetVector() const { return (double *)&p; }
private:
	  int rows;
	  int cols;
